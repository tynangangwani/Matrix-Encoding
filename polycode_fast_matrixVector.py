...
# Input matrix size - A: s by r, B: s by 1
s = 5024
r = 1024
...
prim_root=3
rt= pow(prim_root,int(((F-1)/(m))), F)

var = [pow(rt, i, F) for i in range(length)] + [pow(prim_root, i, F) for i in range(1, (int(N-length))+1) if (i%int(((F-1)/(m)))!=0)]
...
if comm.rank == 0:
  ...
  B = np.matrix(np.random.randint(0,255, (1, s)) )
  ...
  for i in range(N):
    ...
    reqB[i] = comm.Isend([B, MPI.INT], dest=i+1, tag=29)
  ...
  Cver = [(Ap[int(i %m)] * B.getT()) % F for i in range(m * n)]
  ...
  Bi = np.empty_like(np.matrix([[0]*s]))
